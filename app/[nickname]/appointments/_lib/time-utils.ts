...
/**
 * Generate time slots for the grid labels
 * Uses labelIntervalMinutes for spacing (default: hourly)
 */
export function generateTimeSlots(config: GridConfig): TimeSlot[] {
  const slots: TimeSlot[] = [];
  const totalMinutes = (config.endHour - config.startHour) * 60;
  const labelInterval = config.labelIntervalMinutes ?? 60;

  for (let offset = 0; offset <= totalMinutes; offset += labelInterval) {
