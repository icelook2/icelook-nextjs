...
import { minutesToTime, timeToMinutes, timesOverlap } from "./time-utils";
...
export function calculateTimeFromY(
  ...
  const clampedMinutes = Math.max(minMinutes, Math.min(snappedMinutes, maxMinutes));
...
export function checkWithinWorkingHours(
  ...
  for (const breakPeriod of workingDay.breaks) {
    if (
      timesOverlap(startTime, endTime, breakPeriod.start_time, breakPeriod.end_time)
    ) {
