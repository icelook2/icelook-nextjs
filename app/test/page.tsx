...
export default function ExampleCombobox() {
  ...
  return (
    <Combobox.Root items={fruits}>
      <div className="relative flex flex-col gap-1 text-sm leading-5 font-medium text-foreground">
        <label htmlFor={id}>Choose a fruit</label>
        <div className="relative [&>input]:pr-[calc(0.5rem+1.5rem)] has-[.combobox-clear]:[&>input]:pr-[calc(0.5rem+1.5rem*2)]">
          <Combobox.Input
            placeholder="e.g. Apple"
            id={id}
            className="h-10 w-full rounded-md font-normal border border-border pl-3.5 text-base text-foreground bg-background focus:outline focus:outline-2 focus:-outline-offset-1 focus:outline-accent"
          />
          <div className="absolute right-2 bottom-0 flex h-10 items-center justify-center text-muted">
            <Combobox.Clear
      ...
      <Combobox.Portal>
        <Combobox.Positioner className="outline-none" sideOffset={4}>
          <Combobox.Popup className="w-[var(--anchor-width)] max-h-[23rem] max-w-[var(--available-width)] origin-[var(--transform-origin)] rounded-md bg-background text-foreground shadow-lg outline outline-1 outline-border transition-[transform,scale,opacity] data-[ending-style]:scale-95 data-[ending-style]:opacity-0 data-[starting-style]:scale-95 data-[starting-style]:opacity-0 dark:shadow-none duration-100">
            <Combobox.Empty className="p-4 text-[0.925rem] leading-4 text-muted empty:m-0 empty:p-0">
              No fruits found.
            </Combobox.Empty>
