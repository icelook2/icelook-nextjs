...
import {
  ...
  AppointmentsTable,
} from "../clients/[clientId]/appointments/_components";

interface ClientAppointmentsContentProps {
  nickname: string;
  clientId: string;
  backHref: string;
  searchParams: {
...
/**
 * Shared content component for client appointments page.
 * Used by both /clients/[clientId]/appointments and /blocked-clients/[clientId]/appointments routes.
 */
export async function ClientAppointmentsContent({
  nickname,
  clientId,
  backHref,
  searchParams,
}: ClientAppointmentsContentProps) {
  const t = await getTranslations("clients.appointments_page");

  // Validate and parse search params
  const sort = (
    ["date", "service", "price"].includes(searchParams.sort ?? "")
      ? searchParams.sort
      : "date"
