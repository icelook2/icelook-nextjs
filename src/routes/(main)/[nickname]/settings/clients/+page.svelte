<script lang="ts">
	...
	import type { ClientListItem, ClientsResponse } from '$lib/types/client';
	import { Users } from 'lucide-svelte';
	import { formatDistanceToNow } from 'date-fns';

	let { data } = $props();

	let extraClients: ClientListItem[] = $state([]);
	...
	async function loadMore() {
		...
		try {
			const res = await fetch(
				`${env.PUBLIC_API_URL}/specialists/${data.specialistId}/clients?page=${pagination.page + 1}`,
				{ credentials: 'include' }
			);
			if (res.ok) {
				const next: ClientsResponse = await res.json();
...
<header class="flex items-center gap-3">
	...
	<h1 class="text-xl font-semibold text-neutral-900 dark:text-neutral-50">Clients</h1>
</header>
...
{:else}
	<div class="mt-4 divide-y divide-neutral-100 dark:divide-neutral-800">
		{#each clients as client (client.id)}
			<button
				onclick={() => goto(`/${page.params.nickname}/settings/clients/${client.id}`)}
			...
			>
				...
				{#if client.lastAppointmentAt}
					<span class="shrink-0 text-xs text-neutral-400 dark:text-neutral-500">
						{formatDistanceToNow(new Date(client.lastAppointmentAt), { addSuffix: true })}
					</span>
				{/if}
			</button>
		{/each}
